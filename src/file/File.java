...
public class File {
	...
	public List<Item> readFile() throws IOException {
		...
		while ((line = bufferedReader.readLine()) != null) {
			...
			items.add(new Item(lines[0], Integer.parseInt(lines[1]), Integer.parseInt(lines[2]), (Integer.parseInt(lines[2])/Integer.parseInt(lines[1]))));
	...
	public void writeFile(Result result) throws IOException {
		...
		printWriter.println(result.getItems().size());
		for(Item item : result.getItems()) {
			printWriter.println(item.getLabel() + " " + result.getTotalValue());
		}
